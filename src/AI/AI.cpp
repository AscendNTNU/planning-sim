...
action_t AI::chooseAction(Robot target) {
    ...
    // If no target is visible, we do a patrol round
        ...
        action_t search_Action;

        point_t nextSearchPoint = point_Zero;
        ...
        if (x_ > trackCenterX_ && y_ > trackCenterY_) {
            nextSearchPoint.x = trackWidth_ - padding_;
            nextSearchPoint.y = padding_;
        } else if (x_ > trackCenterX_ && y_ < trackCenterY_) {
            nextSearchPoint.x = padding_;
            nextSearchPoint.y = padding_;
        } else if (x_ <= trackCenterX_ && y_ <= trackCenterY_) {
            nextSearchPoint.x = padding_;
            nextSearchPoint.y = trackHeight_ - padding_;
        } else if (x_ < trackCenterX_ && y_ > trackCenterY_) {
            nextSearchPoint.x = trackWidth_ - padding_;
            nextSearchPoint.y = trackHeight_ - padding_;
        }
        ...
        search_Action.type = search;
        search_Action.where_To_Act = nextSearchPoint;
